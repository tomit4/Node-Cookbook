...
function post(req, res) {
    if (req.headers["content-type"] !== "application/x-www-form-urlencoded") { // if the requeste headers are not of this specific content type(a www url-encoded form)...
    ...
    req.on("end", () => { // once the request has received an "end" message...
        console.log(input); // log to the console the data chunk input string.
        res.end(http.STATUS_CODES[200]); // and upon resolution of the POST request, return a successful status code(ie 'OK').
    });
}
